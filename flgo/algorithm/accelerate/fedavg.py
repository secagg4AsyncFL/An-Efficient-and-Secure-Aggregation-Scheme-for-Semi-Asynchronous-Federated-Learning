...
class Server(fedbase.BasicServer):
    ...
    def communicate(self, selected_clients, mtype=0, asynchronous=False):
        ...
        if self.num_parallels <= 1:
            ...
            def client_train_fedavg(data_ref, model, config, calculator):
            ...
            # if not hasattr(self, '_client_data_refs'):
            #     self._client_data_refs = [ray.put(c.train_data) for c in self.clients]
            for client_id in communicate_clients:
                ...
                cpkg = client_train_fedavg.remote(self._client_data_sharable[client_id], model, config, calculator)
