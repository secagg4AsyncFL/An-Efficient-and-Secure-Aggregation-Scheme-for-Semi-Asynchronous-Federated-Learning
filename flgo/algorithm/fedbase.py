...
class BasicServer(BasicParty):
    ...
    def __init__(self, option={}):
        ...
        # global aggregation step size (eta)
        self.eta = option.get('eta', 1.0)  # default eta=1.0 means standard aggregation
        # systemic option
        ...
        self.id = -1
        # timing statistics
        self.round_times = []  # list to store each round's execution time
        self.total_train_time = 0.0  # total training time
        self.round_start_time = 0.0  # start time of current round
    ...
    def run(self):
        ...
        while True:
            ...
            if updated is True or updated is None:
                ...
                self.gv.logger.info("--------------Round {} (Time: {:.2f}s)--------------".format(self.current_round, round_time))
                # check log interval
        ...
        # save timing results as .json file
    ...
    def aggregate(self, models: list, *args, **kwargs):
        ...
        total_data_vol = sum(local_data_vols)
        # Get aggregation step size eta (default 1.0)
        eta = getattr(self, 'eta', 1.0)
        if self.aggregation_option == 'weighted_scale':
            p = [1.0 * local_data_vols[cid] / total_data_vol for cid in self.received_clients]
            K = len(models)
            N = self.num_clients
            aggregated_model = fmodule._model_sum([model_k * pk for model_k, pk in zip(models, p)]) * N / K
            # Apply eta: new_model = old_model + eta * (aggregated_model - old_model)
            return self.model + eta * (aggregated_model - self.model)
        elif self.aggregation_option == 'uniform':
            aggregated_model = fmodule._model_average(models)
            # Apply eta: new_model = old_model + eta * (aggregated_model - old_model)
            return self.model + eta * (aggregated_model - self.model)
        elif self.aggregation_option == 'weighted_com':
            p = [1.0 * local_data_vols[cid] / total_data_vol for cid in self.received_clients]
            w = fmodule._model_sum([model_k * pk for model_k, pk in zip(models, p)])
            aggregated_model = (1.0 - sum(p)) * self.model + w
            # Apply eta: new_model = old_model + eta * (aggregated_model - old_model)
            return self.model + eta * (aggregated_model - self.model)
        else:
            p = [1.0 * local_data_vols[cid] / total_data_vol for cid in self.received_clients]
    ...
    def _save_timing_results(self):
        ...
        # Extract algorithm name from option
        algo_name = self.option.get('algorithm', 'unknown')
        # Extract dataset name from task path
        task_parts = self.option['task'].replace('\\', '/').split('/')
        dataset_name = 'unknown'
        for part in task_parts:
            if part.startswith('test_') or 'mnist' in part.lower() or 'cifar' in part.lower() or 'fashion' in part.lower():
                dataset_name = part
                break
        # Get learning rate
        lr = self.learning_rate
        
        # Create timing results dictionary
        timing_results = {
            'algorithm': algo_name,
            ...
            'average_round_time': sum([r['time'] for r in self.round_times]) / len(self.round_times) if self.round_times else 0,
            'round_times': self.round_times,
            'timestamp': time_module.strftime('%Y%m%d_%H%M%S')
        }
        
        # Create filename
        filename = f"timing_{algo_name}_{dataset_name}_lr{lr}.json"
        filepath = os.path.join(self.option['task'], filename)
        
        # Save to JSON file
        try:
            with open(filepath, 'w') as f:
                json.dump(timing_results, f, indent=4)
            self.gv.logger.info(f"Timing results saved to {filepath}")
            self.gv.logger.info(f"Total training time: {self.total_train_time:.2f}s")
            self.gv.logger.info(f"Average round time: {timing_results['average_round_time']:.2f}s")
        except Exception as e:
            self.gv.logger.warning(f"Failed to save timing results: {str(e)}")
